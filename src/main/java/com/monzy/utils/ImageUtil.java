...
public class ImageUtil {
  ...
  public static void readmob(int zoomlv, int mob) {
    ...
    try {
      DataInputStream dis =
          new DataInputStream(new FileInputStream("data/monzy/mob/x" + zoomlv + "/" + mob));
  ...
  public static void readDataNewBoss(
      byte[] data, int typeRead, BufferedImage image, int zoomLevel, int mobId) {
    try {
      ...
      for (int i = 0; i < nImageInfo; i++) {
        ...
        saveImage(
            imagesInfo[i],
            "C:\\Users\\admin\\Desktop\\read mob\\imginfo\\x" + zoomLevel + "\\" + mobId,
            mobId + "_" + id);
      }
      ...
      for (int i = 0; i < nFrame; i++) {
        ...
        saveImage(
            trimImage(frame),
            "C:\\Users\\admin\\Desktop\\read mob\\frame\\x" + zoomLevel + "\\" + mobId,
            mobId + "_" + i);
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public static void readDataMob(byte[] data, BufferedImage image, int zoomLevel, int mobId) {
    try {
      ...
      for (int i = 0; i < nImageInfo; i++) {
        int id = dis.readByte();
        int x = dis.readUnsignedByte() * zoomLevel;
        int y = dis.readUnsignedByte() * zoomLevel;
        int w = dis.readUnsignedByte() * zoomLevel;
        int h = dis.readUnsignedByte() * zoomLevel;
        //                if (y < 0) {
        //                    y = 0;
        //                }
        //                if (y > image.getHeight()) {
      ...
      ImageOutputStream output =
          new FileImageOutputStream(
              new File("C:\\Users\\admin\\Desktop\\read mob\\framegif\\" + mobId + ".gif"));
  ...
  public static void readEff(int id, int zoom) {
    try {
      DataInputStream dis =
          new DataInputStream(new FileInputStream("data/monzy/effdata/x" + zoom + "/" + id));
  ...
  private static void readDataEffect(byte[] data, int idEffect, int zoom, BufferedImage oriImage) {
    try {
      ...
      for (int i = 0; i < nFrame; i++) {
        ...
        saveImage(
            trimImage(frame),
            "C:\\Users\\admin\\Desktop\\effect_\\x" + zoom + "\\" + idEffect,
            i + "");
      }
