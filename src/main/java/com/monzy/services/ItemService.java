...
public class ItemService {
    ...
    if (item != null) {
      for (Item.ItemOption io : item.itemOptions) {
        if (io.optionTemplate.id == 93) {
          int dayPass =
              (int) TimeUtil.diffDate(new Date(), new Date(item.createTime), TimeUtil.DAY);
          if (dayPass != 0) {
            io.param -= dayPass;
            if (io.param <= 0) {
              return true;
  ...
  public void sendSetTSKichHoat(Player player, int idSKH) {
    ...
    Item ao =
        ItemService.gI().randomCSDTS(Manager.IDS_DO_THIEN_SU[player.gender][0], player.gender);
    Item quan =
        ItemService.gI().randomCSDTS(Manager.IDS_DO_THIEN_SU[player.gender][1], player.gender);
    Item gang =
        ItemService.gI().randomCSDTS(Manager.IDS_DO_THIEN_SU[player.gender][2], player.gender);
    Item giay =
        ItemService.gI().randomCSDTS(Manager.IDS_DO_THIEN_SU[player.gender][3], player.gender);
    Item nhan =
        ItemService.gI().randomCSDTS(Manager.IDS_DO_THIEN_SU[player.gender][4], player.gender);
    ...
    ao.itemOptions =
        ao.itemOptions.stream()
            .filter(itemOption -> itemOption.optionTemplate.id != 30)
  ...
  public Item randomCSDTL(int itemId, int typeDrop) {
    ...
    if (ao.contains(itemId)) {
      item.itemOptions.add(
          new Item.ItemOption(
              47, highlightsItem(item.template.gender == 2, Util.nextInt(1000, 1500))));
    }
    if (quan.contains(itemId)) {
      item.itemOptions.add(
          new Item.ItemOption(22, highlightsItem(item.template.gender == 0, Util.nextInt(45, 60))));
    }
    if (gang.contains(itemId)) {
      item.itemOptions.add(
          new Item.ItemOption(
              0, highlightsItem(item.template.gender == 2, Util.nextInt(3500, 4500))));
    }
    if (giay.contains(itemId)) {
      item.itemOptions.add(
          new Item.ItemOption(23, highlightsItem(item.template.gender == 1, Util.nextInt(45, 60))));
    }
    if (ntl == itemId) {
      item.itemOptions.add(new Item.ItemOption(14, Util.nextInt(15, 18)));
    }
    if (typeDrop == BOSS_DROP) item.itemOptions.add(new Item.ItemOption(209, 1));
    item.itemOptions.add(new Item.ItemOption(21, 18));
    item.itemOptions.add(
        new Item.ItemOption(
            107, typeDrop == BOSS_DROP ? randomStarBossReward() : randomStarModReward()));
    return item;
  }
  ...
  public Item randomCSDHD(int itemId, int gender) {
    ...
    if (ao.contains(itemId)) {
      item.itemOptions.add(
          new Item.ItemOption(47, highlightsItem(gender == 2, Util.nextInt(2000, 2500))));
    }
    if (quan.contains(itemId)) {
      item.itemOptions.add(
          new Item.ItemOption(22, highlightsItem(gender == 0, Util.nextInt(75, 90))));
    }
    if (gang.contains(itemId)) {
      item.itemOptions.add(
          new Item.ItemOption(0, highlightsItem(gender == 2, Util.nextInt(5500, 6500))));
    }
    if (giay.contains(itemId)) {
      item.itemOptions.add(
          new Item.ItemOption(23, highlightsItem(gender == 1, Util.nextInt(75, 90))));
    }
    if (nhd == itemId) {
  ...
  public Item randomCSDTS(int itemId, int gender) {
    ...
    if (ao.contains(itemId)) {
      dots.itemOptions.add(
          new Item.ItemOption(47, highlightsItem(gender == 2, Util.nextInt(3000, 3500))));
    }
    if (quan.contains(itemId)) {
      dots.itemOptions.add(
          new Item.ItemOption(22, highlightsItem(gender == 0, Util.nextInt(105, 120))));
    }
    if (gang.contains(itemId)) {
      dots.itemOptions.add(
          new Item.ItemOption(0, highlightsItem(gender == 2, Util.nextInt(7500, 8500))));
    }
    if (giay.contains(itemId)) {
      dots.itemOptions.add(
          new Item.ItemOption(23, highlightsItem(gender == 1, Util.nextInt(105, 120))));
    }
    if (nhan.contains(itemId)) {
      dots.itemOptions.add(
          new Item.ItemOption(14, highlightsItem(gender == 1, Util.nextInt(23, 26))));
    }
  ...
  public void giaobong(Player player, int quantity) {
    ...
    try {
      ...
      InventoryService.gI().sendItemBags(player);
      Service.gI()
          .sendThongBao(player, "Bạn đã nhận được x" + (quantity / 100) + " " + item.template.name);
    } catch (Exception e) {
      Service.gI().sendThongBao(player, "Bạn không đủ bông...");
    }
  }
  ...
  public List<Item.ItemOption> getListOptionItemShop(short id) {
    ...
    Manager.SHOPS.forEach(
        shop ->
            shop.tabShops.forEach(
                tabShop ->
                    tabShop.itemShops.forEach(
                        itemShop -> {
