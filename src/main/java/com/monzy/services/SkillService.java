...
public class SkillService {
  ...
  public boolean useSkill(Player player, Player plTarget, Mob mobTarget, Message message) {
    ...
    if (player.playerSkill.skillSelect.template.type == 2
    ...
    switch (player.playerSkill.skillSelect.template.type) {
      ...
      case 3:
        ...
        break;
      case 4:
        useSkillSpecial(player, message);
        break;
      default:
        return false;
    }
  ...
  private void useSkillSpecial(Player player, Message message) {
    if (message == null) {
      return;
    }
    try {
    ...
    try {
      ...
      if (player.skillSpecial.skillSpecial.template.id == Skill.MA_PHONG_BA) {
        ...
        if (player.skillSpecial.stepSkillSpecial == 0
            && Util.canDoWithTime(
                player.skillSpecial.lastTimeSkillSpecial, SkillSpecial.TIME_GONG)) {
          player.skillSpecial.lastTimeSkillSpecial = System.currentTimeMillis();
          ...
          if (player.skillSpecial.skillSpecial.template.id == Skill.SUPER_KAME) {
            ...
            && !Util.canDoWithTime(
                player.skillSpecial.lastTimeSkillSpecial,
                player.gender == 1 ? SkillSpecial.TIME_END_26 : SkillSpecial.TIME_END_24_25)) {
          for (Player playerMap : player.zone.getHumanoids()) {
            ...
            if (player.skillSpecial.dir == -1
                ...
                && Math.abs(playerMap.location.x - player.skillSpecial._xPlayer)
                    <= player.skillSpecial._xObjTaget
                && Math.abs(playerMap.location.y - player.skillSpecial._yPlayer)
                    <= player.skillSpecial._yObjTaget
                && this.canAttackPlayer(player, playerMap)) {
          ...
          for (Mob mobMap : player.zone.mobs) {
            ...
            if (player.skillSpecial.dir == -1
                ...
                && Math.abs(mobMap.location.x - player.skillSpecial._xPlayer)
                    <= player.skillSpecial._xObjTaget
                && Math.abs(mobMap.location.y - player.skillSpecial._yPlayer)
                    <= player.skillSpecial._yObjTaget) {
  ...
  public void startSkillSpecialID26(Player player) {
    ...
    try {
      ...
      message.writer().writeByte(player.gender);
      final byte size =
          (byte) (player.skillSpecial.playersTaget.size() + player.skillSpecial.mobsTaget.size());
  ...
  // này hoc5 skill nha
  public void learnSkillSpecial(Player player, byte skillID) {
    Message message = null;
    try {
    ...
    if (!player.isBoss) {
      if (player.isPet) {
        ...
        if (player.nPoint.stamina > 0) {
          ...
          Service.gI()
              .sendThongBao(player, "Thể lực đã cạn kiệt, hãy nghỉ ngơi để lấy lại sức");
          return;
        }
      }
    }
    ...
    switch (player.playerSkill.skillSelect.template.id) {
      case Skill.KAIOKEN: // kaioken
        ...
        if (player.nPoint.hp <= hpUse) {
          ...
          player.nPoint.setHp((long) (player.nPoint.mp - hpUse));
      ...
      case Skill.LIEN_HOAN:
        ...
        if (mobTarget != null
            && Util.getDistance(player, mobTarget) > Skill.RANGE_ATTACK_CHIEU_DAM) {
          miss = true;
        }
      case Skill.KAMEJOKO:
      case Skill.MASENKO:
      ...
      case Skill.QUA_CAU_KENH_KHI:
        if (!player.playerSkill.prepareQCKK) {
          ...
          for (Mob mob : player.zone.mobs) {
            if (!mob.equals(mobTarget)
                && !mob.isDie()
                && Util.getDistance(mob, mobTarget)
                <= SkillUtil.getRangeQCKK(player.playerSkill.skillSelect.point)) {
              mob.injured(player, (int) (player.nPoint.dame * (player.playerSkill.skillSelect.damage / 100f)), true);
            }
          }
          PlayerService.gI().sendInfoHpMpMoney(player);
          afterUseSkill(player, player.playerSkill.skillSelect.template.id);
      ...
      case Skill.DICH_CHUYEN_TUC_THOI:
        ...
        if (plTarget != null) {
          ...
          playerAttackPlayer(player, plTarget, miss);
          EffectSkillService.gI()
              .setBlindDCTT(plTarget, System.currentTimeMillis(), timeChoangDCTT);
          EffectSkillService.gI()
              .sendEffectPlayer(
                  player,
                  plTarget,
      ...
      case Skill.TROI:
        ...
        if (plTarget != null
            && (!plTarget.playerSkill.prepareQCKK
                && !plTarget.playerSkill.prepareLaze
  ...
  private void useSkillAlone(Player player) {
    ...
    switch (player.playerSkill.skillSelect.template.id) {
      case Skill.THAI_DUONG_HA_SAN:
        ...
        for (Mob mob : player.zone.mobs) {
          if (Util.getDistance(player, mob)
  ...
  public void hutHPMP(Player player, int dame, boolean attackMob) {
    int tiLeHutHp = player.nPoint.getTileHutHp(attackMob);
    int tiLeHutMp = player.nPoint.getTiLeHutMp();
    int hpHoi = 0;
    hpHoi += (int) (dame * (tiLeHutHp / 100f));
    int mpHoi = 0;
    mpHoi += (int) (dame * (tiLeHutMp / 100f));
    if (hpHoi > 0 || mpHoi > 0) {
  ...
  private void sendPlayerPrepareSkill(Player player, int afterMilliseconds) {
    Message msg;
    try {
      msg = new Message(-45);
      ...
      msg.writer().writeShort(afterMilliseconds);
  ...
  public void sendPlayerPrepareBom(Player player, int afterMilliseconds) {
    Message msg;
    try {
      msg = new Message(-45);
  ...
  public boolean canUseSkillWithCooldown(Player player) {
    if (!Util.canDoWithTime(player.playerSkill.skillSelect.lastTimeUseThisSkill,
        ...
        switch (skillBefore.template.id) {
          ...
          case Skill.ANTOMIC:
            switch (skill.template.id) {
              ...
              case Skill.GALICK:
              case Skill.ANTOMIC:
                //                                skill.lastTimeUseThisSkill =
                // System.currentTimeMillis() + (skill.coolDown / 100);
                break;
            }
            break;
        }
