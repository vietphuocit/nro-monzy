...
import com.monzy.models.clan.ClanMessage;
...
import static com.monzy.models.clan.Clan.*;
...
public class ClanService {
          ...
          if (cmg.receiveDonate < cmg.maxDonate) {
            ...
            if (plReceive != null) {
              ...
              if (pea != null) {
                ...
                Service.gI()
                    .sendThongBao(plReceive, plGive.name + " đã cho bạn " + peaCopy.template.name);
                cmg.receiveDonate++;
  ...
  private void cancelAskJoinClan(Player player, int clanMessageId) {
    ...
    if (clan != null && clan.isLeader(player)) {
      ...
      if (cmg != null) {
        ...
        if (newMember != null) {
          Service.gI()
              .sendThongBao(newMember, "Bang hội " + clan.name + " đã từ chối bạn vào bang");
        }
        clan.sendMyClanForAllMember();
      }
    }
            ...
            Service.gI()
                .sendThongBao(
                    ...
                    "Bạn không đủ vàng, còn thiếu "
                        + Util.numberToMoney(flagBag.gold - player.inventory.gold)
                        + " vàng");
            return;
          }
        }
            ...
            Service.gI()
                .sendThongBao(
                    ...
                    "Bạn không đủ ngọc, còn thiếu "
                        + (flagBag.gem - player.inventory.gem)
                        + " ngọc");
            return;
          }
        }
  ...
  public void showMenuLeaveClan(Player player) {
    NpcService.gI()
        .createMenuConMeo(
            player,
            ConstNpc.CONFIRM_LEAVE_CLAN,
            -1,
            "Bạn có chắc chắn rời bang hội không?",
            "Đồng ý",
            "Từ chối");
  }
  ...
  public void changeFlag(Player player, int imgId) {
    ...
    if (flagBag.gem > 0 && player.inventory.gem < flagBag.gem) {
      Service.gI()
          .sendThongBao(
              player,
              "Bạn không đủ ngọc, còn thiếu " + (flagBag.gem - player.inventory.gem) + " ngọc");
      return;
    }
    if (flagBag.gold > 0) {
      player.inventory.gold -= flagBag.gold;
    ...
    try (Connection con = Database.getConnection()) {
      ps =
          con.prepareStatement(
              "update player set clan_id_sv" + Manager.SERVER + " = -1 where id = " + plId);
      ps.executeUpdate();
      ps.close();
    } catch (Exception ex) {
