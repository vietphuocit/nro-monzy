...
public class MapService {
  ...
  public WayPoint getWaypointPlayerIn(Player player) {
    for (WayPoint wp : player.zone.map.wayPoints) {
      if (player.location.x >= wp.minX
          && player.location.x <= wp.maxX
          && player.location.y >= wp.minY
  ...
  public int[][] readTileIndexTileType(int tileTypeFocus) {
    ...
    try {
      DataInputStream dis =
          new DataInputStream(new FileInputStream("data/monzy/map/tile_set_info"));
  ...
  public List<Zone> getMapCapsule(Player pl) {
    ...
    if (pl.mapBeforeCapsule != null
        ...
        // && !isMapDiaNguc(pl.mapBeforeCapsule.map.mapId)) {
