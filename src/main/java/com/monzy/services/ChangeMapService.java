...
public class ChangeMapService {
            ...
            switch (pl.iDMark.getTypeChangeMap()) {
                case ConstMap.CHANGE_CAPSULE:
                    ...
                    for (int i = 0; i < pl.mapCapsule.size(); i++) {
                        ...
                        } else if (zone.map.mapName.equals("Nhà Broly")
                                || zone.map.mapName.equals("Nhà Gôhan")
    ...
    public void changeZone(Player player, int zoneId) {
        ...
        if (player.isAdmin()
                || player.isBoss
                || Util.canDoWithTime(player.iDMark.getLastTimeChangeZone(), 10000)) {
            ...
            if (zoneId >= 0 && zoneId <= map.zones.size() - 1) {
                ...
                if (zoneJoin != null
                        && (zoneJoin.getNumOfPlayers() >= zoneJoin.maxPlayer
                        && !player.isAdmin()
                        && !player.isBoss)) {
            ...
            Service.gI()
                    .sendThongBaoOK(
                            player,
                            "Không thể đổi khu vực lúc này, vui lòng đợi "
                                    + TimeUtil.getTimeLeft(player.iDMark.getLastTimeChangeZone(), 10));
        }
    }
    ...
    private void changeMap(
            Player pl, Zone zoneJoin, int mapId, int zoneId, int x, int y, byte typeSpace) {
        ...
        if (zoneJoin != null) {
            ...
            if (typeSpace == AUTO_SPACE_SHIP) {
                spaceShipArrive(
                        pl, (byte) 0, pl.haveTennisSpaceShip ? TENNIS_SPACE_SHIP : DEFAULT_SPACE_SHIP);
