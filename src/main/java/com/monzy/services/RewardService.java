...
import com.monzy.models.Template.ItemLuckyRound;
...
public class RewardService {
  ...
  private static final int[][][] ACTIVATION_SET = {
    {
      {129, 141, 1, 1000}, {127, 139, 1, 1000}, {128, 140, 1, 1000}
    }, // songoku - thien xin hang - kirin
    {
      {131, 143, 1, 1000}, {132, 144, 1, 1000}, {130, 142, 1, 1000}
    }, // oc tieu - pikkoro daimao - picolo
      ...
      if (option_param[i][0] != -1 && option_param[i][1] != -1) {
        list.add(
            new Item.ItemOption(
                ...
                (option_param[i][1]
                    + Util.nextInt(
                        -(option_param[i][1] * 10 / 100), option_param[i][1] * 10 / 100))));
      }
    }
  }
  ...
  public List<Item> getListItemLuckyRound(Player player, int num) {
    ...
    for (int i = 0; i < num; i++) {
      ItemLuckyRound item =
          Manager.LUCKY_ROUND_REWARDS.get(Util.nextInt(0, Manager.LUCKY_ROUND_REWARDS.size() - 1));
