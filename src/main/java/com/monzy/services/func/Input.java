...
//            InventoryService.gI().addItemBag(playerNap, traiDua);
        ...
        case TAIHN:
          ...
          Thread threadTaiHN = new Thread(() -> {
            int timeSeconds = 10;
            Service.gI().sendThongBao(player, "Chờ " + timeSeconds + " giây để biết kết quả.");
            while (timeSeconds > 0) {
              timeSeconds--;
              try {
                Thread.sleep(1000);
        ...
        case XIUHN:
          ...
          Thread threadXiuHN = new Thread(() -> {
            ...
            if (3 <= (x + y + z) && (x + y + z) <= 10) {
              ...
              Service.gI().sendMoney(player);
              Service.gI().sendThongBaoOK(player, "Kết quả" + "\nSố hệ thống quay ra : " + x + " " + y + " " + z + "\nTổng là : " + tong + "\nBạn đã cược : " + xiuHongNgoc + " Hồng Ngọc vào Xỉu" + "\nKết quả : Xỉu" + "\n\nVề bờ");
        ...
        case TAITV:
          ...
          Thread threadTaiTV = new Thread(() -> {
            ...
            int tong = (x + y + z);
            if (3 <= tong && tong <= 10) {
              Service.gI().sendThongBaoOK(player, "Kết quả" + "\nSố hệ thống quay ra là :" + " " + x + " " + y + " " + z + "\nTổng là : " + tong + "\nBạn đã cược : " + taiThoiVang + " Thỏi vàng vào Tài" + "\nKết quả : Xỉu" + "\nCòn cái nịt.");
        ...
        case XIUTV:
          ...
          Thread threadXiuTV = new Thread(() -> {
            ...
            int tong = (x + y + z);
            if (tong > 10 && tong <= 18) {
              Service.gI().sendThongBaoOK(player, "Kết quả" + "\nSố hệ thống quay ra là :" + " " + x + " " + y + " " + z + "\nTổng là : " + tong + "\nBạn đã cược : " + xiuThoiVang + " Thỏi vàng vào Xỉu" + "\nKết quả : Tài" + "\nCòn cái nịt.");
        ...
        case CHOOSE_LEVEL_BDKB:
          ...
          if (level >= 1 && level <= 110) {
            ...
            if (npc != null) {
              npc.createOtherMenu(player, ConstNpc.MENU_ACCEPT_GO_TO_BDKB, "Con có chắc chắn muốn tới bản đồ kho báu cấp độ " + level + "?", new String[]{"Đồng ý", "Từ chối"}, level);
            }
          } else {
        ...
        case VND_TO_HONG_NGOC:
          ...
          if (goldTrade <= 0 || goldTrade >= 50000000) {
            ...
            Service.gI().sendThongBao(player, "Số tiền của bạn là " + player.session.vnd + " không đủ để quy " + " đổi " + goldTrade + " Hồng Ngọc " + " " + "bạn cần thêm" + (player.session.vnd - goldTrade));
          }
          break;
        case VND_TO_THOI_VANG:
          ...
          if (gemTrade <= 0 || gemTrade >= 50000000) {
            ...
            InventoryService.gI().addItemBag(player, thoiVang);
            InventoryService.gI().sendItemBags(player);
            Service.gI().sendThongBao(player, "bạn nhận được " + gemTrade * ratioGem + " " + thoiVang.template.name);
          } else {
            Service.gI().sendThongBao(player, "Số tiền của bạn là " + player.session.vnd + " không đủ để quy " + " đổi " + gemTrade + " Thỏi Vàng" + " " + "bạn cần thêm" + (player.session.vnd - gemTrade));
          }
          break;
      }
    } catch (Exception e) {
  ...
  public void createFormChangePassword(Player pl) {
    createForm(pl, CHANGE_PASSWORD, "Quên Mật Khẩu", new SubInput("Nhập mật khẩu đã quên", PASSWORD), new SubInput("Mật khẩu mới", PASSWORD), new SubInput("Nhập lại mật khẩu mới", PASSWORD));
  }
  ...
  public void createFormSendRuby(Player pl) {
    createForm(pl, SEND_RUBY, "Gửi hồng ngọc", new SubInput("Tên", ANY), new SubInput("Số lượng", ANY));
  }

  public void createFormSendItem(Player pl) {
    createForm(pl, SEND_ITEM, "Gửi vật phẩm", new SubInput("Tên", ANY), new SubInput("ID vật phẩm", ANY), new SubInput("Số lượng", ANY));
  }
