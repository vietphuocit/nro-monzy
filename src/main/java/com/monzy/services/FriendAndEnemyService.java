...
import com.monzy.models.player.Enemy;
...
public class FriendAndEnemyService {
  ...
  private void reloadFriend(Player player) {
    for (Friend f : player.friends) {
      Player pl = null;
      if ((pl = Client.gI().getPlayerByUser(f.id)) != null
          || (pl = Client.gI().getPlayer(f.name)) != null) {
        try {
  ...
  private void removeFriend(Player player, int playerId) {
    for (int i = 0; i < player.friends.size(); i++) {
      if (player.friends.get(i).id == playerId) {
        Service.gI()
            .sendThongBao(
                player, "Đã xóa thành công " + player.friends.get(i).name + " khỏi danh sách bạn");
        Message msg;
        try {
          msg = new Message(-80);
        ...
        if (player.isAdmin() || player.nPoint.teleport) {
          if (!pl.itemTime.isUseAnDanh || player.isAdmin()) {
            if ((player.isAdmin() || !pl.zone.isFullPlayer())
              ...
              ChangeMapService.gI()
                  .changeMapYardrat(
                      player, pl.zone, pl.location.x + Util.nextInt(-5, 5), pl.location.y);
            } else {
              Service.gI().sendThongBao(player, "Không thể thực hiện");
            }
          } else {
