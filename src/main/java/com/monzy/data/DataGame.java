...
public class DataGame {
    ...
    public static void updateData(MySession session) {
        ...
        byte[] image = FileIO.readFile("data/monzy/update_data/image");
        byte[] part = FileIO.readFile("data/monzy/update_data/part");
        byte[] skill = FileIO.readFile("data/monzy/update_data/skill");
        Message msg;
        try {
            msg = new Message(-87);
            msg.writer().writeByte(vsData);
    ...
    public static void sendDataImageVersion(MySession session) {
        ...
        try {
            msg = new Message(-111);
            msg.writer().write(FileIO.readFile("data/monzy/data_img_version/x" + session.zoomLevel + "/img_version"));
            session.doSendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
            Logger.logException(DataGame.class, e);
    ...
    public static void sendEffectTemplate(MySession session, int id) {
        Message msg;
        try {
            byte[] eff_data = FileIO.readFile("data/monzy/effdata/x" + session.zoomLevel + "/" + id);
            msg = new Message(-66);
    ...
    public static void effData(MySession session, int id, int... idtemp) {
        ...
        Message msg;
        try {
            byte[] effData = FileIO.readFile("data/monzy/effect/x" + session.zoomLevel + "/data/DataEffect_" + idT);
            byte[] effImg = FileIO.readFile("data/monzy/effect/x" + session.zoomLevel + "/img/ImgEffect_" + idT + ".png");
            msg = new Message(-66);
    ...
    public static void sendItemBGTemplate(MySession session, int id) {
        Message msg;
        try {
            byte[] bg_temp = FileIO.readFile("data/monzy/item_bg_temp/x" + session.zoomLevel + "/" + id + ".png");
            msg = new Message(-32);
            msg.writer().writeShort(id);
    ...
    public static void sendDataItemBG(MySession session) {
        Message msg;
        try {
            byte[] item_bg = FileIO.readFile("data/monzy/item_bg_temp/item_bg_data");
            msg = new Message(-31);
    ...
    public static void sendIcon(MySession session, int id) {
        Message msg;
        try {
            byte[] icon = FileIO.readFile("data/monzy/icon/x" + session.zoomLevel + "/" + id + ".png");
            msg = new Message(-67);
    ...
    public static void sendSmallVersion(MySession session) {
        ...
        try {
            msg = new Message(-77);
            byte[] data = FileIO.readFile("data/monzy/data_img_version/x" + session.zoomLevel + "/img_version");
            if (data != null) {
...
//        }
        Message msg;
        try {
            byte[] mob = FileIO.readFile("data/monzy/mob/x" + session.zoomLevel + "/" + id);
            msg = new Message(11);
    ...
    public static void sendTileSetInfo(MySession session) {
        ...
        try {
            msg = new Message(-82);
            msg.writer().write(FileIO.readFile("data/monzy/map/tile_set_info"));
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
        }
    ...
    public static void mainz(String[] args) {
        try {
            File folder = new File("data/monzy/map/tile_map_data");
    ...
    public static void mainzz(String[] args) throws Exception {
        ...
        File folder = new File("C:\\Users\\admin\\Desktop\\nro qltk java by girlkun\\monzy\\map\\tile_map_data");
//        for (File f : folder.listFiles()) {
//            list.add(f.getName());
//        }
        folder = new File("C:\\Users\\admin\\Desktop\\cbro\\data\\monzy\\map\\tile_map_dataz");
        for (File f : folder.listFiles()) {
            if (!list.contains(f.getName())) {
                ...
                dis.close();
                DataOutputStream dos = new DataOutputStream(new FileOutputStream("C:\\Users\\admin\\Desktop\\cbro\\data\\monzy\\map\\tile_map_data\\" + f.getName()));
                dos.writeByte(w);
                dos.writeByte(h);
                for (int i = 0; i < data.length; i++) {
    ...
    public static void sendMapTemp(MySession session, int id) {
        ...
        try {
            ...
            msg.writer().write(FileIO.readFile("data/monzy/map/tile_map_data/" + id));
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
            Logger.logException(DataGame.class, e);
    ...
    public static void sendImageByName(MySession session, String imgName) {
        ...
        try {
            ...
            byte[] data = FileIO.readFile("data/monzy/img_by_name/x" + session.zoomLevel + "/" + imgName + ".png");
    ...
    public static void sendSizeRes(MySession session) {
        ...
        try {
            ...
            msg.writer().writeShort(new File("data/monzy/res/x" + session.zoomLevel).listFiles().length);
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
        }
    ...
    public static void sendRes(MySession session) {
        Message msg;
        try {
            for (final File fileEntry : new File("data/monzy/res/x" + session.zoomLevel).listFiles()) {
