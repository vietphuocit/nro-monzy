...
public class DataGame {
  ...
  public static void sendDataImageVersion(MySession session) {
    ...
    try {
      ...
      msg.writer()
          .write(
              FileIO.readFile(
                  "data/monzy/data_img_version/x" + session.zoomLevel + "/img_version"));
      session.doSendMessage(msg);
      msg.cleanup();
    } catch (Exception e) {
      Logger.logException(DataGame.class, e);
  ...
  public static void effData(MySession session, int id, int... idtemp) {
    ...
    try {
      byte[] effData =
          FileIO.readFile("data/monzy/effect/x" + session.zoomLevel + "/data/DataEffect_" + idT);
      byte[] effImg =
          FileIO.readFile(
              "data/monzy/effect/x" + session.zoomLevel + "/img/ImgEffect_" + idT + ".png");
      msg = new Message(-66);
  ...
  public static void sendItemBGTemplate(MySession session, int id) {
    ...
    try {
      byte[] bg_temp =
          FileIO.readFile("data/monzy/item_bg_temp/x" + session.zoomLevel + "/" + id + ".png");
      msg = new Message(-32);
      msg.writer().writeShort(id);
  ...
  public static void sendSmallVersion(MySession session) {
    ...
    try {
      msg = new Message(-77);
      byte[] data =
          FileIO.readFile("data/monzy/data_img_version/x" + session.zoomLevel + "/img_version");
      if (data != null) {
        ...
        DataOutputStream dos =
            new DataOutputStream(
                new FileOutputStream(
                    "C:\\Users\\admin\\Desktop\\cbro\\data\\monzy\\map\\tile_map_data\\"
                        + f.getName()));
        dos.writeByte(w);
        dos.writeByte(h);
        for (int i = 0; i < data.length; i++) {
  ...
  public static void sendImageByName(MySession session, String imgName) {
    ...
    try {
      ...
      byte[] data =
          FileIO.readFile("data/monzy/img_by_name/x" + session.zoomLevel + "/" + imgName + ".png");
