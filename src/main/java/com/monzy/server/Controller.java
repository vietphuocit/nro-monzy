...
//          break;
        ...
        case 7: // sell item
          if (player != null && !Maintenance.isRunning) {
            ...
            if (action == 0) {
              ShopServiceNew.gI()
                  .showConfirmSellItem(player, _msg.reader().readByte(), _msg.reader().readShort());
            } else {
              ShopServiceNew.gI()
                  .sellItem(player, _msg.reader().readByte(), _msg.reader().readShort());
            }
          }
          break;
        case 29:
    ...
    if (TaskService.gI().getIdTask(player) == ConstTask.TASK_0_0) {
      NpcService.gI()
          .createTutorial(
              ...
              "Chào mừng "
                  + player.name
                  + " đến với ngọc rồng online server MONZY\n"
  ...
  private void sendThongBaoServer(Player player) {
    Service.gI()
        .sendThongBaoFromAdmin(player, "Quay về Đây Em cầm tay.\nEm khóc trên đôi vai này...");
  }

  private void clearVTSK(Player player) {
    player.inventory.itemsBag.stream()
        .filter(item -> item.isNotNullItem() && item.template.id == 610)
        .forEach(item -> InventoryService.gI().subQuantityItemsBag(player, item, item.quantity));
    player.inventory.itemsBox.stream()
        .filter(item -> item.isNotNullItem() && item.template.id == 610)
        .forEach(item -> InventoryService.gI().subQuantityItemsBox(player, item, item.quantity));
