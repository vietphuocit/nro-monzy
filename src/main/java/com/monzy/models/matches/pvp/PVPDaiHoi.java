...
public class PVPDaiHoi extends PVP {
  ...
  public void update() {
    if (Util.canDoWithTime(lastTimePVP, 180000)) {
      if (p1.nPoint.hp > p2.nPoint.hp) {
        ...
        Service.gI()
            .sendThongBao(
                p1, "Trận đấu hòa. Bạn nhận được " + Util.numberToMoney(this.goldReward) + " vàng");
        ...
        Service.gI()
            .sendThongBao(
                p2, "Trận đấu hòa. Bạn nhận được " + Util.numberToMoney(this.goldReward) + " vàng");
  ...
  public void sendResult(Player plLose, TYPE_LOSE_PVP typeLose) {
    if (typeLose == TYPE_LOSE_PVP.RUNS_AWAY) {
      Service.gI()
          .sendThongBao(
              p1.equals(plLose) ? p2 : p1,
              "Đối thủ kiệt sức. Bạn thắng nhận được "
                  + Util.numberToMoney(this.goldReward)
                  + " vàng");
