...
public class Pet extends Player {
  ...
  private static final short[][] PET_ID = {
    {285, 286, 287}, {288, 289, 290}, {282, 283, 284}, {304, 305, 303}
  };
    ...
    if (Util.canDoWithTime(lastTimeUnfusion, TIME_WAIT_AFTER_UNFUSION)) {
      if (porata) {
        ...
        ItemTimeService.gI()
            .sendItemTime(
                master,
                master.gender == ConstPlayer.NAMEC ? 3901 : 3790,
  ...
  private void moveIdle() {
    ...
    if (idle && Util.canDoWithTime(lastTimeMoveIdle, timeMoveIdle)) {
      ...
      PlayerService.gI()
          .playerMove(
              this,
              master.location.x + Util.nextInt(dir == -1 ? 30 : -50, dir == -1 ? 50 : 30),
              master.location.y);
  ...
  public void update() {
    try {
      ...
      switch (status) {
            ...
            if (disToMob <= ARANGE_ATT_SKILL1) {
              ...
              if (SkillService.gI().canUseSkillWithCooldown(this)) {
                if (SkillService.gI().canUseSkillWithMana(this)) {
                  PlayerService.gI()
                      .playerMove(
                          this, mobAttack.location.x + Util.nextInt(-20, 20), mobAttack.location.y);
                ...
                if (SkillService.gI().canUseSkillWithCooldown(this)) {
                  if (SkillService.gI().canUseSkillWithMana(this)) {
                    PlayerService.gI()
                        .playerMove(
                            this,
                            mobAttack.location.x + Util.nextInt(-20, 20),
        ...
        case GOHOME:
          if (this.zone != null
              && (this.zone.map.mapId == 21
                  || this.zone.map.mapId == 22
            ...
            if (System.currentTimeMillis() - lastTimeMoveAtHome <= 5000) {
              ...
              Service.gI()
                  .chatJustForMe(master, this, "H2O + C12H22O11 -> Uống ngọt lắm sư phụ ạ!");
  ...
  public void askPea() {
    if (Util.canDoWithTime(lastTimeAskPea, 10000)) {
      Service.gI()
          .chatJustForMe(master, this, "Sư phụ ơi cho con đậu thần đi, con đói sắp chết rồi !!");
  ...
  public void openSkill5() {
    ...
    if (rd <= tiLeThoiMien) {
      ...
      skill = SkillUtil.createSkill(Skill.THOI_MIEN, 7);
    } else if (rd <= tiLeThoiMien + tiLeSoCoLa + tiLeDCTT) {
      skill = SkillUtil.createSkill(Skill.DICH_CHUYEN_TUC_THOI, 7);
    }
