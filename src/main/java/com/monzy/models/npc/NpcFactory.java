...
public class NpcFactory {
  ...
  private static Npc ongGohan_ongMoori_ongParagus(int mapId, int status, int cx, int cy, int tempId, int avatar) {
  ...
  private static Npc kyGui(int mapId, int status, int cx, int cy, int tempId, int avatar) {
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          if (!TaskService.gI().checkDoneTaskTalkNpc(player, this)) {
            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Cửa hàng chúng tôi chuyên mua bán hàng hiệu, hàng độc, cảm ơn bạn đã ghé thăm.", "Mua bán\nKý gửi", "Từ chối");
          }
        }
      }
      ...
      public void confirmMenu(Player player, int select) {
        if (canOpenNpc(player) && this.mapId == 140 && player.iDMark.isBaseMenu()) {
          if (select == 0) {
            ...
            List<Skill> skillList = player.playerSkill.skills.stream().filter(skill -> skill.point > 0).collect(Collectors.toList());
            int[][] skillTemp = skillList.stream().filter(skill -> skill.point > 0).map(skill -> new int[]{skill.template.id, skill.point, skill.coolDown}).toArray(int[][]::new);
            BossData bossDataClone = new BossData("Nhân Bản " + player.name, player.gender, new short[]{player.getHead(), player.getBody(), player.getLeg(), player.getFlagBag(), player.idAura, player.getEffFront()}, player.nPoint.dame, new int[]{player.nPoint.hpMax}, new int[]{140}, skillTemp, new String[]{"|-1|Vào mà húp"}, // text chat 1
  ...
  private static Npc quyLaoKame(int mapId, int status, int cx, int cy, int tempId, int avatar) {
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          if (!TaskService.gI().checkDoneTaskTalkNpc(player, this)) {
            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Chào con, ta rất vui khi gặp con\n Con muốn làm gì nào ?", "Nói chuyện", "Bảng\nXếp Hạng");
          }
        }
      }
      ...
      public void confirmMenu(Player player, int select) {
        if (canOpenNpc(player)) {
          if (player.iDMark.isBaseMenu()) {
            switch (select) {
              case 0: {
                this.createOtherMenu(player, ConstNpc.QUY_LAO_KAME, "Chào con, ta rất vui khi gặp con\n Con muốn làm gì nào ?", "Về khu\nvực bang", "Giải tán\nBang hội", "Kho Báu\ndưới biển");
                return;
              }
              case 1: {
          ...
          if (player.iDMark.getIndexMenu() == ConstNpc.QUY_LAO_KAME) {
            switch (select) {
              ...
              case 1: {
                ...
                NpcService.gI().createMenuConMeo(player, ConstNpc.CONFIRM_DISSOLUTION_CLAN, -1, "Con có chắc chắn muốn giải tán bang hội không? Ta cho con 2 lựa chọn...", "Đồng ý", "Từ chối!");
                return;
              }
              case 2: {
                if (player.clan == null) {
                ...
                if (player.clan.banDoKhoBau == null) {
                  this.createOtherMenu(player, ConstNpc.MENU_OPEN_DBKB, "Đây là bản đồ kho báu x4 tnsm\nCác con cứ yên tâm lên đường\n" + "Ở đây có ta lo\nNhớ chọn cấp độ vừa sức mình nhé", "Chọn\ncấp độ", "Từ chối");
                  return;
                }
                this.createOtherMenu(player, ConstNpc.MENU_OPENED_DBKB, "Bang hội của con đang đi tìm kho báu dưới biển cấp độ " + player.clan.banDoKhoBau.getLevel() + "\nCon có muốn đi theo không?", "Đồng ý", "Từ chối");
                return;
              }
            }
            return;
          ...
          if (player.iDMark.getIndexMenu() == ConstNpc.MENU_OPENED_DBKB) {
            if (select == 0) {
              if (player.nPoint.power < BanDoKhoBauService.POWER_HAS_GO_TO_DBKB) {
                this.npcChat(player, "Sức mạnh của con phải ít nhất phải đạt " + Util.numberToMoney(BanDoKhoBauService.POWER_HAS_GO_TO_DBKB));
                return;
              }
          ...
          if (player.iDMark.getIndexMenu() == ConstNpc.MENU_ACCEPT_GO_TO_BDKB) {
            if (select == 0) {
              BanDoKhoBauService.gI().openBanDoKhoBau(player, Byte.parseByte(String.valueOf(PLAYER_ID_OBJECT.get(player.id))));
            }
          }
        }
      }
    ...
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void confirmMenu(Player player, int select) {
        if (canOpenNpc(player)) {
          ...
          if (player.iDMark.getIndexMenu() == 1 && player.zone.map.mapId == 7) {
            switch (select) {
              case 0:
                this.npcChat("Chưa có hướng dẫn");
                break;
              case 1:
                ...
                if (player.clan == null) {
                  Service.gI().sendThongBao(player, "Anh phải có bang hội mới tập hợp đủ 7 viên ngọc rồng Namek");
                  return;
                }
//                List<Player> players =
//                    player.clan.membersInGame.stream().filter(player1 -> player1.zone.equals(player.zone)).collect(Collectors.toList());
//                HashSet<Short> idsNRNM = new HashSet<>();
//                for (Player player1 : players) {
//                  if (player1.idNRNM != -1) {
//                    idsNRNM.add(player1.idNRNM);
//                  }
//                }
//
//                if (idsNRNM.size() < 7) {
//                  Service.gI().sendThongBao(player, "Bang của anh chưa tập hợp đủ 7 viên ngọc rồng Namek");
//                  return;
//                }
            ...
            if (player.iDMark.getIndexMenu() == ConstNpc.MENU_FIND_KUKU) {
              switch (select) {
                case 0:
                  ...
                  Zone zone = MapService.gI().getMapCanJoin(player, kuku.zone.map.mapId, kuku.zone.zoneId);
            ...
            if (player.iDMark.getIndexMenu() == ConstNpc.MENU_FIND_MAP_DAU_DINH) {
              switch (select) {
                case 0:
                  ...
                  Zone zone = MapService.gI().getMapCanJoin(player, mapDauDinh.zone.map.mapId, mapDauDinh.zone.zoneId);
                  if (zone == null || zone.getNumOfPlayers() >= zone.maxPlayer) {
                    Service.gI().sendThongBao(player, "Khu vực này đã đầy");
                    return;
                  }
            ...
            if (player.iDMark.getIndexMenu() == ConstNpc.MENU_FIND_RAMBO) {
              switch (select) {
                case 0:
                  ...
                  Zone zone = MapService.gI().getMapCanJoin(player, rambo.zone.map.mapId, rambo.zone.zoneId);
                  if (zone == null || zone.getNumOfPlayers() >= zone.maxPlayer) {
                    Service.gI().sendThongBao(player, "Khu vực này đã đầy");
                    return;
                  }
  ...
  public static Npc santa(int mapId, int status, int cx, int cy, int tempId, int avatar) {
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          createOtherMenu(player, ConstNpc.BASE_MENU, "Xin chào, ta có một số vật phẩm đặt biệt cậu có muốn xem không?", "Cửa hàng", "Đổi Tiền", "Shop Hồng Ngọc");
        }
      }
            ...
            if (player.iDMark.isBaseMenu()) {
              switch (select) {
                case 0: // shop
                  ...
                  break;
                case 1:
                  this.createOtherMenu(player, ConstNpc.QUY_DOI, "|7|Số tiền của bạn còn : " + player.session.vnd + "\nTỉ lệ quy đổi là 1-1" + "\nMuốn quy đổi không", "Quy đổi\nHồng ngọc", "Quy Đổi\nThỏi vàng");
                  break;
                case 2: // shop
  ...
  public static Npc thoDaiKa(int mapId, int status, int cx, int cy, int tempId, int avatar) {
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          createOtherMenu(player, ConstNpc.BASE_MENU, "Đưa cho ta Hồng Ngọc hoặc Thỏi Vàng và ngươi sẽ mua đc oto\nĐây không phải chẵn lẻ tài xỉu đâu=)))", "Tài\n hồng ngọc", "Xỉu\n hồng ngọc", "Tài\n thỏi vàng", "Xỉu\n thỏi vàng");
        }
      }
    ...
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          ...
          if (this.mapId == 121) {
            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Ngươi tìm ta có việc gì?", "Về\nđảo rùa");
            return;
          }
            ...
            if (player.iDMark.getIndexMenu() == ConstNpc.MENU_START_COMBINE) {
              switch (player.combine.typeCombine) {
                ...
                case CombineService.PHA_LE_HOA_TRANG_BI:
                  switch (select) {
                    case 0:
                      if(CombineService.gI().phaLeHoaTrangBi(player)) {
                    ...
                    case 1: {
                      for (int i = 0; i < 10; i++) {
                        Boolean check = CombineService.gI().phaLeHoaTrangBi(player);
                        if (check == null) {
                          break;
                        }
                        if (check) {
                          Service.gI().sendThongBao(player, "Nâng cấp thành công sau " + (i + 1) + " lần đập");
                          CombineService.gI().sendEffectSuccessCombine(player);
                          break;
                        }
                        if(i == 9) {
...
//                case CombineService.NANG_CAP_DO_TS:
        ...
        if (canOpenNpc(player)) {
          if (mapId == 0) {
            ...
            if (player.nPoint.power >= 1_500_000L && player.nPoint.power <= 20_000_000_000L) {
              this.createOtherMenu(player, 0, "Ngũ Hành Sơn x10 Tnsm\nHỗ trợ cho Ae từ 1tr5 đến 20tỷ sức mạnh\n250 hồng ngọc 1 lần vào", "Đồng ý", "Từ chối");
              return;
            }
            this.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Sức mạnh bạn không đủ điều kiện\nChỉ hỗ trợ cho Ae 1tr5 đến 20tỷ sức mạnh.", "Không");
          }
          if (mapId == 123) {
  ...
  public static Npc calick(int mapId, int status, int cx, int cy, int tempId, int avatar) {
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        ...
        if (this.mapId == 102) {
          this.createOtherMenu(player, ConstNpc.BASE_MENU, "Chào chú, cháu có thể giúp gì?", "Kể\nChuyện", "Quay về\nQuá khứ");
          return;
        }
        this.createOtherMenu(player, ConstNpc.BASE_MENU, "Chào chú, cháu có thể giúp gì?", "Kể\nChuyện", "Đi đến\nTương lai", "Từ chối");
      }
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          if (this.mapId == 24 || this.mapId == 25 || this.mapId == 26) {
            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Gô Tên, Calich và Monaka đang gặp chuyện ở hành tinh Potaufeu \n Hãy đến đó ngay", "Đến \nPotaufeu");
          }
          if (this.mapId == 139) {
            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Người muốn trở về?", "Quay về", "Từ chối");
          }
        }
      }
  ...
  public static Npc npcLyTieuNuong54(int mapId, int status, int cx, int cy, int tempId, int avatar) {
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      @Override
      public void openBaseMenu(Player player) {
  ...
  public static Npc thuongDe(int mapId, int status, int cx, int cy, int tempId, int avatar) {
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          if (this.mapId == 45) {
            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Con muốn làm gì nào", "Đến Kaio", "Quay số\nmay mắn");
          }
        }
      }
            ...
            if (player.iDMark.isBaseMenu()) {
              switch (select) {
                case 0:
                  ...
                  return;
                case 1:
                  this.createOtherMenu(player, ConstNpc.MENU_CHOOSE_LUCKY_ROUND, "Con muốn làm gì nào?", "Quay bằng\nvàng", "Quay bằng\n hồng ngọc", "Rương phụ\n(" + (player.inventory.itemsBoxCrackBall.size() - InventoryService.gI().getCountEmptyListItem(player.inventory.itemsBoxCrackBall)) + " món)", "Xóa hết\ntrong rương", "Đóng");
                  return;
              }
            }
            if (player.iDMark.getIndexMenu() == ConstNpc.MENU_CHOOSE_LUCKY_ROUND) {
              switch (select) {
                ...
                case 2:
                  ...
                  return;
                case 3:
                  NpcService.gI().createMenuConMeo(player, ConstNpc.CONFIRM_REMOVE_ALL_ITEM_LUCKY_ROUND, this.avartar, "Con có chắc muốn xóa hết vật phẩm trong rương phụ?\nSau khi xóa sẽ không thể khôi phục!", "Đồng ý", "Hủy bỏ");
                  return;
              }
            }
          }
      ...
      public void confirmMenu(Player player, int select) {
        if (canOpenNpc(player)) {
          if (this.mapId == 48) {
            if (player.iDMark.isBaseMenu()) {
              if (select == 0) {
                this.createOtherMenu(player, ConstNpc.MENU_DI_CHUYEN, "Con muốn đi đâu?", "Về\nthần điện", "Thánh địa\nKaio", "Con\nđường\nrắn độc", "Từ chối");
              }
            }
  ...
  public static Npc kibit(int mapId, int status, int cx, int cy, int tempId, int avatar) {
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          if (this.mapId == 50) {
            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Ta có thể giúp gì cho ngươi ?", "Đến\nKaio", "Từ chối");
            return;
          }
    ...
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          ...
          if (this.mapId == 154) {
            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Ta có thể giúp gì cho ngươi ?", "Về thánh địa", "Đến\nhành tinh\nngục tù", "Từ chối");
          }
          if (this.mapId == 155) {
          ...
          if (this.mapId == 52) {
            ...
            if (now > MapMaBu.TIME_OPEN_MABU && now < MapMaBu.TIME_CLOSE_MABU) {
              this.createOtherMenu(player, ConstNpc.MENU_OPEN_MMB, "Đại chiến Ma Bư đã mở, ngươi có muốn tham gia không?", "Hướng dẫn", "Tham gia", "Từ chối");
              return;
            }
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          ...
          if (player.clan.getMembers().size() < DoanhTraiDocNhanService.N_PLAYER_IN_CLAN) {
            this.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Bang hội phải có ít nhất " + DoanhTraiDocNhanService.N_PLAYER_IN_CLAN + " thành viên mới có thể mở", "Đóng");
            return;
          }
          if (player.clan.doanhTraiDocNhan != null) {
            createOtherMenu(player, ConstNpc.MENU_JOIN_DOANH_TRAI, "Bang hội của ngươi đang đánh trại độc nhãn\n" + "Thời gian còn lại là " + TimeUtil.getSecondLeft(player.clan.lastTimeOpenDTDN, DoanhTraiDocNhanService.TIME_DOANH_TRAI / 1000) + ". Ngươi có muốn tham gia không?", "Tham gia", "Không", "Hướng\ndẫn\nthêm");
            return;
          }
          int nPlSameClan = 0;
          ...
          if (nPlSameClan < DoanhTraiDocNhanService.N_PLAYER_IN_MAP) {
            createOtherMenu(player, ConstNpc.IGNORE_MENU, "Ngươi phải có ít nhất " + DoanhTraiDocNhanService.N_PLAYER_IN_MAP + " đồng đội cùng bang đứng gần mới có thể\nvào\n" + "tuy nhiên ta khuyên ngươi nên đi cùng với 3-4 người để khỏi chết.\n" + "Hahaha.", "OK", "Hướng\ndẫn\nthêm");
            return;
          }
          if (player.clanMember.getNumDateFromJoinTimeToToday() < 1) {
            createOtherMenu(player, ConstNpc.IGNORE_MENU, "Doanh trại chỉ cho phép những người ở trong bang trên 1 ngày. Hẹn ngươi quay lại vào lúc khác", "OK", "Hướng\ndẫn\nthêm");
            return;
          }
          if (!player.clan.hasGoneOpenDoanhTrai()) {
            createOtherMenu(player, ConstNpc.IGNORE_MENU, "Bang hội của ngươi đã đi trại lúc " + TimeUtil.formatTime(player.clan.lastTimeOpenDTDN, "HH:mm:ss") + " hôm nay. Hẹn ngươi quay lại vào ngày mai", "OK", "Hướng\ndẫn\nthêm");
            return;
          }
          ...
          if (this.mapId == 154) {
            switch (player.iDMark.getIndexMenu()) {
              case ConstNpc.CAN_NOT_OPEN_BILL:
                if (select == 0) {
                  this.createOtherMenu(player, ConstNpc.CONFIRM_DESTROY_BILL, "Bạn có chắc chắn muốn hủy bỏ trứng Mabư?", "Đồng ý", "Từ chối");
                } else if (select == 1) {
                  if (player.inventory.gold >= COST_AP_TRUNG_NHANH) {
                    player.inventory.gold -= COST_AP_TRUNG_NHANH;
              ...
              case ConstNpc.CAN_OPEN_EGG:
                switch (select) {
                  case 0:
                    this.createOtherMenu(player, ConstNpc.CONFIRM_OPEN_BILL, "Bạn có chắc chắn cho trứng nở?\n" + "Đệ tử của bạn sẽ được thay thế bằng đệ Mabư", "Đệ mabư\nTrái Đất", "Đệ mabư\nNamếc", "Đệ mabư\nXayda", "Từ chối");
                    break;
                  case 1:
                    this.createOtherMenu(player, ConstNpc.CONFIRM_DESTROY_BILL, "Bạn có chắc chắn muốn hủy bỏ trứng Mabư?", "Đồng ý", "Từ chối");
                    break;
            ...
            switch (select) {
              case 0:
                if (player.nPoint.limitPower < NPoint.MAX_LIMIT) {
                  this.createOtherMenu(player, ConstNpc.OPEN_POWER_MYSEFT, "Ta sẽ truyền năng lượng giúp con mở giới hạn sức mạnh của bản thân lên " + Util.numberToMoney(player.nPoint.getPowerNextLimit()), "Nâng\ngiới hạn\nsức mạnh", "Nâng ngay", "Đóng");
                  return;
                }
              ...
              case 1:
                ...
                if (player.pet.nPoint.limitPower < NPoint.MAX_LIMIT) {
                  this.createOtherMenu(player, ConstNpc.OPEN_POWER_PET, "Ta sẽ truyền năng lượng giúp con mở giới hạn sức mạnh của đệ tử lên " + Util.numberToMoney(player.pet.nPoint.getPowerNextLimit()), "Nâng ngay", "Đóng");
                  return;
                }
          ...
          if (this.mapId == 24 || this.mapId == 25 || this.mapId == 26) {
            try {
              ...
              if (now > BlackBallWar.TIME_OPEN && now < BlackBallWar.TIME_CLOSE) {
                ...
                int index = 0;
                for (int i = 0; i < 7; i++) {
                  if (player.rewardBlackBall.rewardsExpire[i] > System.currentTimeMillis()) {
                    String quantily = player.rewardBlackBall.quantityBlackBall[i] > 1 ? "x" + player.rewardBlackBall.quantityBlackBall[i] + " " : "";
    ...
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void confirmMenu(Player player, int select) {
        if (canOpenNpc(player)) {
          ...
          if (player.iDMark.getIndexMenu() == ConstNpc.MENU_OPTION_GO_HOME) {
            if (select == 0) {
          ...
          if (player.iDMark.getIndexMenu() == ConstNpc.MENU_OPTION_PHU_HP) {
...
//                  }
  ...
  public static Npc bill(int mapId, int status, int cx, int cy, int tempId, int avatar) {
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          createOtherMenu(player, ConstNpc.BASE_MENU, "Ngươi chỉ cần mang Mảnh Chiến Lực đến đây\n Ta sẽ giúp ngươi có được những trang bị\n xịn nhất của ta!", "Shop Bill", "Đóng");
        }
      }
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          if (this.mapId == 47 || this.mapId == 84) {
            this.createOtherMenu(player, ConstNpc.BASE_MENU, "|2|Hông nàm mà đoài cóa ăn thì chỉ cóa ăn \b|7|dau buoi an cut!", "Nhiệm vụ\nhàng ngày", "Từ chối");
          }
        }
      }
            ...
            if (player.iDMark.isBaseMenu()) {
              if (select == 0) {
                if (player.playerTask.sideTask.template != null) {
                  ...
                  this.createOtherMenu(player, ConstNpc.MENU_OPTION_PAY_SIDE_TASK, npcSay, "Trả nhiệm\nvụ", "Hủy nhiệm\nvụ");
                } else {
                  this.createOtherMenu(player, ConstNpc.MENU_OPTION_LEVEL_SIDE_TASK, "Tôi có vài nhiệm vụ theo cấp bậc, " + "sức cậu có thể làm được cái nào?", "Dễ", "Bình thường", "Khó", "Siêu khó", "Địa ngục", "Từ chối");
                }
              }
  ...
  public static Npc vados(int mapId, int status, int cx, int cy, int tempId, int avatar) {
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          createOtherMenu(player, ConstNpc.BASE_MENU, "|2|Ta Vừa Hắc Mắp Xêm Được Tóp Của Toàn Server\b|7|Người Muốn Xem Tóp Gì?", "TOP Sức Mạnh", "TOP Nhiệm Vụ", "TOP NẠP", "Đóng");
        }
      }
  ...
  public static Npc mavuong(int mapId, int status, int cx, int cy, int tempId, int avatar) {
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          if (this.mapId == 153) {
            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Xin chào, tôi có thể giúp gì cho cậu?", "Tây thánh địa", "Từ chối");
  ...
  public static Npc khidaumoi(int mapId, int status, int cx, int cy, int tempId, int avatar) {
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        if (this.mapId == 14) {
          this.createOtherMenu(player, ConstNpc.BASE_MENU, "Bạn muốn nâng cấp khỉ ư?", "Nâng cấp\nkhỉ", "Shop của Khỉ", "Từ chối");
        }
      }
  ...
  public static Npc unknow(int mapId, int status, int cx, int cy, int tempId, int avatar) {
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          if (this.mapId == 5) {
            this.createOtherMenu(player, 0, "Éc éc Bạn muốn gì ở tôi :3?", "Đến Võ đài Unknow", "Đổi Rương Đồng Vàng", "Hướng Dẫn Sự Kiện");
          }
          if (this.mapId == 112) {
            this.createOtherMenu(player, 0, "Bạn đang còn : " + player.pointPvp + " điểm PvP Point", "Về đảo Kame", "Đổi Cải trang sự kiên", "Top PVP");
          }
        }
      }
  ...
  public static Npc monaito(int mapId, int status, int cx, int cy, int tempId, int avatar) {
    return new Npc(mapId, status, cx, cy, tempId, avatar) {
      ...
      public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
          if (this.mapId == 7) {
            this.createOtherMenu(player, 0, "Chào bạn tôi sẽ đưa bạn đến hành tinh Cereal?", "Đồng ý", "Từ chối");
          }
          if (this.mapId == 170) {
      ...
      switch (tempId) {
        ...
        default:
          return new Npc(mapId, status, cx, cy, tempId, avatar) {
            ...
            @Override
            public void confirmMenu(Player player, int select) {
              canOpenNpc(player); //                                ShopService.gI().openShopNormal(player,
