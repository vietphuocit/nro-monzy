...
public class Boss extends Player implements IBossNew, IBossOutfit {
  ...
  private final Map<Integer, Integer> ITEM_REWARD =
  ...
  private final Map<Integer, Integer> MANH_THIEN_SU =
      new HashMap<Integer, Integer>() {
        {
          put(1066, 33);
          put(1067, 33);
  ...
  public Player getPlayerAttack() {
    if (this.playerTarget != null
        && (this.playerTarget.isDie() || !this.zone.equals(this.playerTarget.zone))) {
    ...
    if (this.zone != null) {
      if (this.currentLevel == 0) {
        if (this.parentBoss == null) {
          ...
          ChangeMapService.gI()
              .changeMapBySpaceShip(
                  this, this.zone, this.parentBoss.location.x + Util.nextInt(-100, 100));
        }
  ...
  public void chatM() {
    ...
    String textChat =
        this.data[this.currentLevel]
            .getTextM()[Util.nextInt(0, this.data[this.currentLevel].getTextM().length - 1)];
    int prefix = Integer.parseInt(textChat.substring(1, textChat.lastIndexOf("|")));
    textChat = textChat.substring(textChat.lastIndexOf("|") + 1);
  ...
  public void die(Player plKill) {
    if (plKill != null) {
      reward(plKill);
      ServerNotify.gI()
          .notify(plKill.name + " vừa tiêu diệt được " + this.name + ", ghê chưa ghê chưa..");
    }
  ...
  public void rewardDTL(Player plKill) {
    ...
    if (Util.isTrue(15, 100)) {
      ...
      ItemMap itemMap =
          new ItemMap(zone, randomIdDoThan, 1, plKill.location.x, plKill.location.y, plKill.id);
  ...
  public void rewardManhThienSu(Player plKill) {
    for (Map.Entry<Integer, Integer> entry : MANH_THIEN_SU.entrySet()) {
      if (Util.isTrue(entry.getValue(), 100)) {
        ItemMap it =
            new ItemMap(
                this.zone, entry.getKey(), 1, plKill.location.x, plKill.location.y, plKill.id);
        Service.gI().dropItemMap(this.zone, it);
        return;
      }
    }
