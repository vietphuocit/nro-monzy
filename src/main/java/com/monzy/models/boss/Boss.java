...
public class Boss extends Player implements IBossNew, IBossOutfit {
	...
	public Zone getMapJoin() {
		...
		if (!zone.getBosses().isEmpty()) return getMapJoin();
		return zone;
	}
	...
	protected void notifyJoinMap() {
		if (this.id >= -22 && this.id <= -20 || this.zone.map.mapId == 140 || MapService.gI().isMapMaBu(this.zone.map.mapId) || MapService.gI().isMapBlackBallWar(this.zone.map.mapId) || MapService.gI().isMapDoanhTrai(this.zone.map.mapId) || MapService.gI().isMapBanDoKhoBau(this.zone.map.mapId))
			return;
...
//    }
	...
	public void wakeupAnotherBossWhenAppear() {
		if (!MapService.gI().isMapMaBu(this.zone.map.mapId) && !MapService.gI().isMapBlackBallWar(this.zone.map.mapId) && !MapService.gI().isMapBanDoKhoBau(this.zone.map.mapId) && !MapService.gI().isNguHS(this.zone.map.mapId)) {
			Logger.log("BOSS " + this.name + " : " + this.zone.map.mapName + " khu vực " + this.zone.zoneId + "(" + this.zone.map.mapId + ")");
		}
		if (this.bossAppearTogether == null || this.bossAppearTogether[this.currentLevel] == null) {
			return;
		}
	...
	public boolean isPersonalBoss(int mapId) {
		return MapService.gI().isMapDoanhTrai(mapId) || MapService.gI().isMapBanDoKhoBau(mapId) || MapService.gI().isMapBlackBallWar(mapId) || 140 == mapId;
	}
